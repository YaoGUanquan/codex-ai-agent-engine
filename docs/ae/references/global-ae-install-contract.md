...
- `$HOME/.cursor/skills/ae-*` is the Cursor user-level discovery surface. Apply copies each current-release skill directory from `$HOME/plugins/ai-agent-engine-codex/skills/<name>` into a real directory at that path. Cursor does not track skill-directory symlinks or junctions, so the installer never publishes those as the discovery surface. A leftover 0.3.29 link that still resolves to the matching personal-plugin skill is treated as historical-release verified and replaced with a matching copy without `--retire-modified`. The installer never writes `$HOME/.cursor/skills-cursor` and never replaces unrelated Cursor skills.
...
- Skill discovery is evaluated at client startup. A new Codex task and a new Cursor chat are required after installation. `codex plugin list` confirms Codex registration; isolated-home tests confirm Cursor skill copies; a dispatcher smoke confirms project-root routing.
