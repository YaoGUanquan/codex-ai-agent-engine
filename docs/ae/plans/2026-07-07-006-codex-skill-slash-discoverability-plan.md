...
## Post-Completion Distribution Patch

Date: 2026-07-07.

An external target-project test found that `ae-tools help` advertised `node scripts/check-ae-artifacts.mjs [--target <project>]`, but the installed project did not receive that wrapper and the plugin package did not contain `plugins/ai-agent-engine-codex/scripts/check-ae-artifacts.mjs`. Direct execution in the target project failed with `MODULE_NOT_FOUND`.

Root cause: the repository-level checker existed only at `scripts/check-ae-artifacts.mjs`; unlike `check-design-contract`, it had not been moved behind a plugin script plus project-level wrapper pattern. The capability catalog claim was valid for this repository but not for installed target projects.

Patch result:

- Added `plugins/ai-agent-engine-codex/scripts/check-ae-artifacts.mjs` as the packaged implementation.
- Converted `scripts/check-ae-artifacts.mjs` into a wrapper that imports the packaged plugin script.
- Updated `scripts/install-project.mjs` to install `scripts/check-ae-artifacts.mjs` into target projects.
- Updated `scripts/check-install-smoke.mjs` to assert both installed paths and actual target-project execution.
- Updated `tests/skill-scripts.test.mjs` so install smoke reports `check-ae-artifacts` in `verifiedCommands`.
- Updated `package.json` check coverage to syntax-check the packaged plugin script.

Patch validation:

- Red test: `npm test -- --test-name-pattern "check-install-smoke reports ok"` failed before the fix because `verifiedCommands` did not include `check-ae-artifacts`.
- Green test: the same command passed after the fix.
- Full validation passed: `npm test`, `npm run check`, `node scripts/check-ae-artifacts.mjs`, `node plugins/ai-agent-engine-codex/scripts/check-ae-artifacts.mjs`, and `git diff --check`.

Updated distribution rule: every command listed in `ae-help/references/capability-catalog.json` as `node scripts/<name>.mjs` must either be generated by `scripts/install-project.mjs` and covered by `scripts/check-install-smoke.mjs`, or the catalog must not advertise it as a target-project command.
