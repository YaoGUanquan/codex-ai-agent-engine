...
- Date: 2026-08-11
- Decision: Move the complete release history (0.3.7-0.3.24) to root `CHANGELOG.md`/`CHANGELOG.en.md`; each README keeps only the latest five entries plus a changelog link, enforced by `scripts/check-release-notes.mjs` (four-file current entry, window limit, link, README-subset-of-changelog). Archive the four finished or pointer directories out of `docs/00-process/active/` in a manual one-time pass. Keep legacy frontend stack guidance on-demand with no pre-filled Svelte 4 / NgModule / Options API entries. Defer the frontend quality-contract mapping note and contract check.
- Context: README carried 18 bilingual version entries and kept growing; tidy intentionally keeps archived-pointer directories and does not classify unlabeled finished tasks, so the closeout cannot come from the shared tool; the frontend contract correspondence is small (about five groups across `web-ui-quality.md`, the `ae-review` Frontend Components / Styles lens, and `browser-acceptance.md`), partially test-locked for motion/reduced-motion keywords, and has had zero drift since 0.3.18/0.3.19.
- Impact: Repository-side only (no plugin content change, no version bump). New checker contract is TDD-covered in `tests/contracts.test.mjs`; the AGENTS.md release rule, `docs/release-checklist.md`, and both README working-rule paragraphs describe the README+CHANGELOG dual-write; `docs/00-process/active/` now only holds in-flight work. PRD `docs/ae/prds/2026-08-11-governance-batch-four-prd.md`; plan `docs/ae/plans/2026-08-11-006-governance-batch-four-plan.md`; experience `docs/ae/experience/2026-08-11-governance-batch-four.md`; archive `docs/00-process/archive/2026-08/governance-batch-four/summary.md`.
- Re-evaluate when: a real legacy-stack project shows one attributable checklist miss (add minimal counterpart entries for that framework and bump the version), a fourth frontend contract file appears or a real drift defect occurs (write a lightweight mapping note first), or release practice shows the five-entry README window is the wrong size.

## 2026-08-11: Close roadmap items 7 and 10 early with legacy counterparts and a descriptive contract map

- Date: 2026-08-11
- Decision: At the user's direction, execute the prescribed actions of roadmap items 7 and 10 without waiting for the recorded triggers: add minimal legacy counterpart sections (Svelte 4 stores, NgModule era, Options API) to the three `ae-web-app` guidance files, and write the descriptive map `docs/ae/references/frontend-quality-contract-map.md` for the three frontend quality contracts. The map stays repository-side and descriptive; no validation script is added, and the `ae-review` Frontend Components / Styles lens stays version-agnostic and unchanged.
- Context: Governance batch four deferred both items behind observable triggers. The 2026-08-11 re-check showed no trigger had fired (125/125 tests, three contract files, no attributable legacy-stack miss); the user then chose early completion over continued dormancy.
- Impact: Distributable version 0.3.26. PRD `docs/ae/prds/2026-08-11-frontend-legacy-and-contract-map-prd.md`; plan `docs/ae/plans/2026-08-11-008-frontend-legacy-and-contract-map-plan.md`; experience `docs/ae/experience/2026-08-11-frontend-legacy-and-contract-map.md`; archive `docs/00-process/archive/2026-08/frontend-legacy-and-contract-map/summary.md`. A regression test locks the counterpart sections and mirrors.
- Re-evaluate when: a real legacy-stack project shows the minimal counterparts are insufficient (extend only the affected framework), or a real drift defect or contract-surface growth justifies upgrading the descriptive map to a checked contract.
