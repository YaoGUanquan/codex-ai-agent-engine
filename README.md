...
CodeBuddy compatibility note: the long-lived `codebuddy` branch adds `plugins/ai-agent-engine-codex/.codebuddy-plugin/plugin.json` so CodeBuddy can discover the same AE `skills/` directory. CodeBuddy-specific updates stay on that branch and are not intended to merge back into `main`.
...
CodeBuddy note: on the `codebuddy` branch, the copied plugin also contains `plugins/ai-agent-engine-codex/.codebuddy-plugin/plugin.json`. Point CodeBuddy local plugin configuration at `./plugins/ai-agent-engine-codex` when using that branch.
