...
2. Recover existing docs/ae artifacts from the target project root with `node "$aeEntry" recovery`; use the selected project wrapper or current-user dispatcher, not a guessed script under the `ae-lfg` skill directory.
