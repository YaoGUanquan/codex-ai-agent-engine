...
初始化当前项目的 Codex 说明、过程文档、归档规则和长期 AI 记忆：
...
这个命令会生成 `AGENTS.md`、`docs/ae`、`docs/00-process`、`docs/08-ai-memory`，并保留 `docs/ai-memory` 作为兼容说明入口。生成的中文文档统一按 UTF-8 读写；如果 PowerShell 里看起来乱码，先用显式 UTF-8 读取验证，不要直接把控制台乱码当成文件损坏。
...
```text
使用 ae-tools init 为当前项目创建 AGENTS.md、docs/00-process、docs/08-ai-memory，并按 UTF-8 规则生成中文模板
```
...
docs/00-process/                 # 过程文档、归档规则和执行模板
docs/08-ai-memory/               # 标准长期 AI 记忆库
docs/ai-memory/                  # 兼容说明入口
```
...
- 生成中文文档时统一按 UTF-8 读写；PowerShell 乱码必须先用显式 UTF-8 读取确认。
