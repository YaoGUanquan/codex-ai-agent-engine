...
import {
  ...
  isOutsideWorktree,
} from '../utils/media-path-security.js'

const DEFAULT_MAX_FILE_BYTES = 100 * 1024 * 1024

export interface MediaFileLoaderResult {
  filePath: string
  buffer: Buffer
  fileSize: number
...
function resolveMaxBytes(): number {
  const envValue = process.env.AE_MEDIA_MAX_BYTES
 ...
 */
export async function resolveMediaPath(file: string, worktree: string): Promise<string> {
...
export async function loadMediaFile(
  ...
  worktree: string,
  formatName: string,
): Promise<MediaFileLoaderResult> {
  let realTarget: string
  try {
    realTarget = await resolveMediaPath(file, worktree)
  } catch (e: unknown) {
