...
export function isProjectPluginInstall(manifest: ReturnType<typeof createRuntimeAssetManifest>, hostWorktree: string): boolean {
    const pluginRoot = resolve(manifest.repoRoot)
    const worktree = resolve(hostWorktree)

    return isSamePath(pluginRoot, worktree)
        || isInsideRoot(join(worktree, '.opencode', 'plugins'), pluginRoot)
        || isInsideRoot(join(worktree, '.opencode', 'ai-agent-engine'), pluginRoot)
}
...
const plugin: Plugin = async (input) => {
    ...
    setGlobalClient(createOpencodeClient(clientConfig))
