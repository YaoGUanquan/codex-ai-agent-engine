...
- 浏览器能力必须通过 `ae:playwright` 技能和 Playwright CLI 执行。CLI 不可用时停止浏览器流程并记录无法验证，不提供绕过方式。
...
- 新增浏览器命令或自动化调用；浏览器流程仍必须使用 `ae:playwright` 技能。
...
浏览器代理或命令的提示词必须要求通过 `ae:playwright` 技能执行 Playwright CLI。CLI 不可用时不得继续浏览器流程。
