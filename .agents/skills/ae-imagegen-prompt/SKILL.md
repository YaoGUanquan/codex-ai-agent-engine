...
4. In beginner mode or missing-config mode, infer use case, aspect ratio, output count, and prompt language; ask at most one clarification if the output use is unclear.
...
- Do not run GUI image editing or Computer Use under this prompt-specification skill; route it through a separately authorized workflow.
- For storyboard assets, produce stable filenames and structured fields that a separately approved media workflow can consume.
