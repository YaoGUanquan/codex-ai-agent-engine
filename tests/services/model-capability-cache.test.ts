...
const NONE_SUPPORTED = { image: false, audio: false, video: false }
...
describe('mimeToModality', () => {
  ...
  it('非媒体 MIME 返回 undefined', () => {
    expect(mimeToModality('application/pdf')).toBeUndefined()
...
describe('getCapability', () => {
  ...
  it('已缓存模型返回实际能力', async () => {
    ...
    expect(caps1).toEqual({ image: true, audio: false, video: false })
...
describe('cacheSessionModel + getCapabilityBySession', () => {
  ...
  it('缓存 sessionID 后查询到对应模型能力', async () => {
    ...
    expect(caps).toEqual({ image: true, audio: false, video: false })
  })
...
describe('extractCapsFromModel', () => {
  it('从完整 Model 对象提取能力', () => {
    const model = {
      ...
      modelID: 'm1',
      capabilities: {
        input: { image: true, audio: false, video: false },
      },
    }
  ...
  it('input 中字段为非布尔值时视为 false', () => {
    const model = {
      capabilities: { input: { image: 'yes', audio: 1, video: null } },
    }
    const caps = extractCapsFromModel(model)
    expect(caps).toEqual({ image: false, audio: false, video: false })
  })
})
...
describe('cacheSessionCapabilities + getCapabilityBySession', () => {
  ...
  it('直接缓存优先于 sessionModel 映射', async () => {
    ...
    cacheSessionCapabilities('session-1', { image: true, audio: false, video: false })
  ...
  it('LRU 淘汰：直接缓存超过上限时删除最旧条目', () => {
    ...
    for (let i = 0; i < 1001; i++) {
      cacheSessionCapabilities(`session-${i}`, { image: true, audio: false, video: false })
    }
