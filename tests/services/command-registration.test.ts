...
describe('command-registration', () => {
  ...
  it('应该为 ae:playwright 生成基础命令', () => {
    const config = buildCommandConfig('__missing_commands_dir__')

    expect(config[COMMAND.PLAYWRIGHT]?.template).toContain(SKILL.PLAYWRIGHT)
  })
  ...
  it('应该保持 ae:playwright catalog 与 SKILL.md frontmatter 语义一致', () => {
    const content = readFileSync('src/assets/skills/ae-playwright/SKILL.md', 'utf8')
    const frontmatter = parseFrontmatter(content).data
    const entry = getPhaseOneEntries().find((item) => item.skillName === SKILL.PLAYWRIGHT)

    expect(frontmatter.description).toContain('Playwright')
