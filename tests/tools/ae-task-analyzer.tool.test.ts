...
beforeAll(() => {
  ...
  createTestFile('config/settings.yaml', 'enabled: true')
  createTestFile('config/schema.json', '{"type":"object"}')
...
describe('ae-task-analyzer 工具', () => {
  describe('mode=scan', () => {
    ...
    it('应该识别 JSON 和 YAML 配置文件', async () => {
      const tool = await getTool()
      const result = await tool.execute(
        { mode: 'scan', task_description: '修改 config/settings.yaml 和 config/schema.json', worktree },
        createMockContext(),
      )
      const parsed = JSON.parse(result) as { units: Array<{ files: Array<{ path: string }> }> }
      const allFiles = parsed.units.flatMap((unit) => unit.files.map((file) => file.path))

      expect(allFiles).toContain('config/settings.yaml')
      expect(allFiles).toContain('config/schema.json')
    })
  })
