...
2. Recover existing docs/ae artifacts from the target project root with `node "$HOME/.agents/ai-agent-engine-codex/bin/ae.mjs" recovery`; the command is installed as the project-level `scripts/ae-tools.mjs` wrapper, not as a file under the `ae-lfg` skill directory.
