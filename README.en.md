...
The **2026-08-11 fullstack skill symmetry release (0.3.21)** landed backend language guidance, FE/BE contract checklist, debug boundary maps, and SQL safety checklist; see `docs/ae/experience/2026-08-11-fullstack-skill-optimization.md`. The **2026-08-11 frontend skill releases (0.3.18 / 0.3.19)** are documented in `docs/ae/experience/2026-08-11-frontend-skill-optimization.md`. Items 7 and 10 below are **on-demand dormant items**: their policies and triggers are settled, and there is no active to-do until a trigger fires. A 2026-08-11 re-check confirmed neither trigger has fired (`npm test` 125/125 green, still three frontend contract files, no attributable legacy-stack miss reported):

7. **Older frontend stack adaptation** (policy set, on-demand): `svelte-guidance.md` and `angular-guidance.md` baseline on Svelte 5 runes and Angular standalone/signals, and the Vue guidance centers on the Composition API; repositories on Svelte 4 stores, NgModules, or the Options API keep the "match existing repository style" fallback. Do not pre-fill legacy counterpart entries; when a real legacy-stack project shows one attributable checklist miss, add the minimal counterpart entries for the affected framework only (a distributable change that bumps the version at that time). Trigger signals live in the 2026-08-11 frontend stack guidance decision in `docs/08-ai-memory/05-decision-log.md`.
...
10. **Cross-consistency of frontend quality contracts** (evaluated, deferred): the accessibility baseline in `web-ui-quality.md`, the Frontend Components / Styles lens in `ae-review`, and the keyboard-operability item in `browser-acceptance.md` share about five correspondence groups; the motion/reduced-motion keywords are already locked by `tests/skills-docs.test.mjs`, and no drift has occurred since their 0.3.18/0.3.19 introduction, so no mapping note or contract check is added now. Revisit when a fourth frontend contract file appears, a real drift defect occurs, or frontend entries grow in bulk again; start with a lightweight mapping note rather than a checker. The evaluation record lives in the 2026-08-11 governance-batch-four entry of `docs/08-ai-memory/05-decision-log.md`.
